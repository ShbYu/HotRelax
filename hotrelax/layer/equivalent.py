...
class SelfInteractionLayer(nn.Module):
    def __init__(
        ...
        output_dim: int = 10,
        bias: bool = True,
        adddim_way: Dict[int, int] = {0: 0, 1: 0, 2: 0, 3: 0},
    ) -> None:
        super().__init__()
        # only the way 0 can have bias
        self.linear_list = nn.ModuleList(
            [
                nn.Linear(input_dim + adddim_way[way], output_dim, bias=(way == 0 and bias))
                for way in range(max_way + 1)
            ]
        )
        self.adddim_way = adddim_way
    ...
    def forward(
        ...
        for way, linear in enumerate(self.linear_list):
            if way in input_tensors:
                if way == 0 and self.adddim_way[0]:
                    idx_j = batch_data["idx_j"]
                    lattice_t = batch_data["cell_u"] @ batch_data["cell_u"].transpose(-1, -2)
                    lattice_t = lattice_t.repeat_interleave(batch_data["n_atoms"], dim=0).view(-1, 9)
                    input_tensors[way] = torch.cat([input_tensors[way], lattice_t[idx_j]], dim=1)
                if way == 1 and self.adddim_way[1]:
                    idx_j = batch_data["idx_j"]
                    lattice = batch_data["cell_u"]
                    lattice = lattice.repeat_interleave(batch_data["n_atoms"], dim=0)
                    input_tensors[way] = torch.cat([input_tensors[way], lattice[idx_j]], dim=1)
