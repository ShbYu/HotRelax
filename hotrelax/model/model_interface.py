...
class LitAtomicModule(pl.LightningModule):
    ...
    def get_loss_calculator(self):
        ...
        if train_dict["loss"] == "mae":
            loss_fn = torch.nn.L1Loss()
        elif train_dict["loss"] == "mse":
            loss_fn = F.mse_loss
        ...
        if train_dict['allowMissing']:
            ...
            return MissingValueLoss(weights, loss_fn=loss_fn)
        elif train_dict['huberDelta'] > 0:
            return MACEHuberLoss(weights, huber_delta=train_dict['huberDelta'])
        else:
            if train_dict['forceScale'] > 0:
                return ForceScaledLoss(weights, loss_fn=loss_fn, scaled=train_dict['forceScale'])
            else:
                return Loss(weights, loss_fn=loss_fn)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for prop in loss_dict:
            ...
            self.log(f'mae_{prop}', mae_dict[prop], batch_size=batch['n_atoms'].shape[0])
